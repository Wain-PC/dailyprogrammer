...
const getPatterns = async () =>
  (await get("./363/patterns.txt"))
    ...
    .map(pattern => {
      ...
      const hyphens = [];
      const text = pattern.replace(/\./g, "").replace(/\d+?/g, (n, o) => {
...
const getTrie = async () =>
  (await getPatterns()).reduce((accumulator, props) => {
...
const solve = (input, patterns) => {
  ...
  patterns.forEach(({ starts, ends, text, hyphens }) => {
...
const solveTrie = (input, tree) => {
  ...
  for (let i = 0; i < split.length; i++) {
    ...
    split.slice(i).every(letter => {
      ...
      if (t[letter].matches) {
        t[letter].matches.forEach(({ starts, ends, text, hyphens }) => {
          if (
            (starts && i === 0) ||
