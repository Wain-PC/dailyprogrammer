...
describe("360.2", () => {
  const examples = [["48.8584 N", "2.2945 E"], ["40.6413 N", "73.7781 W"]];
  ...
  examples.forEach(([lat, lon]) => {
    ...
    it(`'bonus' and 'solve' outputs should be the same (input '${lat} : ${lon}')`, async () => {
      const { distance: distanceSolve, ...solveData } = await solve(
        flights,
        lat,
        lon
      );
      const { distance: distanceBonus, ...bonusData } = await bonus(
        flights,
        lat,
        lon
      );
